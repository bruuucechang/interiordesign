...
export function bounds(o: Obj): Bounds {
  switch (o.kind) {
    ...
    case 'wall': case 'beam': case 'partition': case 'dimension':
...
function hitObject(o: Obj, p: Vec, tol: number): boolean {
  switch (o.kind) {
    ...
    // A partition has no thickness at all, so the only thing to pick is the
    // line itself — the same generous tolerance a dimension gets.
    case 'partition': return distToSegment(p, o.a, o.b) <= tol * 1.5;
    case 'room': {
