...
同一輪順手把**圖層的顯示／鎖定／排序也變成一步歷史**。它們本來只 mutate 不 commit，
而 `layers` 一直都在快照裡——於是最糟的兩半同時成立：隱藏一個圖層自己不可復原，但
**別的東西**的復原會安靜地把它改回去。使用者那一側看到的是：鎖了圖層、按 Ctrl+Z
想解鎖，不見的卻是剛剛畫的那道牆。`importImage` 的 `commit()` 因此要排在動圖層
**之前**，不然匯入底圖會變成兩步。
...
`openModal` 在沒有任何存活方案時 `return`，而 `renderBin()` 在那一行後面。於是唯一
真的需要回收桶的時刻——把手上唯一那張圖刪掉、想救回來——正好是它不存在的時刻，而
新手教學還在旁邊說刪掉的東西 30 天內都救得回來。畫面上沒有錯誤，只有一句「尚無已
儲存的專案」。

**方案是照 owner 分的**（`X-Owner`，存在 localStorage），所以「清單是空的」比想像
中容易發生：換一個瀏覽器、清一次 localStorage 就是一個新 owner。

`bench/verify-bin.mjs` 從頭走一遍（建立 → 刪除 → 在空清單下找到回收桶 → 還原），
還原修改之後那一條會紅。

### 桌面版的語言不能靠 `navigator.language` 猜

`navigator.language` 講的是**瀏覽器**的語言，不是那個人的語言，而兩者常常不一致：
很多看中文的人用的是英文版 Chrome，因為電腦出廠就是那樣。原本 `detect()` 只看它，
於是一台英文 Chrome 會用**英文**跑那八步新手教學——給一個「英文不好」的收件者。

桌面版是交給一個特定的人、由知道他讀什麼的人交出去的，所以 `desktop.py` 開的網址
帶 `?lang=zh-Hant`，把答案講明白而不是讓前端去猜。順序是
**使用者選過的 > 啟動器指定的 > 瀏覽器**，而且第一次決定出來就寫進 localStorage
（不然從書籤開、少了參數，語言會自己跳回去）。

**賭注是不對稱的，這才是這個順序的理由**：英文那一側本來就還沒翻完（家具與材質名、
快捷鍵表、提示列都還是中文），猜錯成英文＝交給他一個半調子介面；猜錯成中文＝他多
按一次那顆一直都在的切換鈕。

`chooseLang()` 抽成純函式才測得到——`navigator` 與 `location` 在 Node 底下是唯讀的
全域，測試沒辦法指派給它們，而這正是「在建置的機器上看不出來、只有在收件者那台才
會出事」的那種決定。

### 空間健檢：一個會唸對的東西的清單，等於沒有清單

參考 homeplan.tw 之後做的（2026-09-08）。它的價值不在「有幾條規則」，在**每一條都
指名一個量到的數字、說出後果，而且不會自己跳出來**。

三條界線寫在 `design-rules.md` 第十節，這裡只留最容易做錯的那一條：**只檢查目錄上
明確標了需求的品項**。通用的「走道要 70cm」聽起來最有用，實際上是誤報製造機——沙發
離茶几 30cm 是對的，而任何不知道那是茶几的規則都會唸它。使用者學會忽略整個清單之後，
真的抓到問題的那一次也會一起被忽略。所以 `health.test.ts` 裡每一條「該抓到」旁邊都
有一條「不該抓到」，那一半比另一半重要。

同一道縫**只報一次**：兩端各報一次的話，按一下「接上」數字會掉 2，看起來像按錯了。
...
2026-09-09 讓 Codex 把這個 repo 從規則檔讀到程式碼跑了一遍。下面每一條都**再自己
查證過**才寫進來 —— 代理人的自評不能當結論用。狀態欄的意思：
`文件已改`＝這一節已經照現況更新；`待修`＝程式碼要動；`待查`＝還沒確認影響範圍。

> **查證後排除的**（Codex 講錯或不精確，寫在這裡免得下次再查一次）：
> · **九個專案裡唯一一個「沒有發現」。** 抽查的六條規則全部與程式碼相符：
> 　家具貼牆轉向與例外（`client/src/tools/place.ts:104`）、動態牆厚門檻（`server/app/detect.py:90`）、
> 　柱框折疊成實心柱（`client/src/ui/properties.ts:758`）、schema 版本仍為 1 且遷移集中管理
> 　（`client/src/model/migrate.ts:24`）、後端入口只有 wiring（`server/app/main.py`）、
> 　寫寬鬆讀嚴格（`server/app/plan.py:67`）。
