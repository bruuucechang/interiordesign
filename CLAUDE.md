...
`fitFurnitureToWall` 只在游標離牆 60cm 內才動作，而且它做的是「貼上去」。離牆遠
一點、用拖曳的、按方向鍵推、或是把東西轉個角度，都可以把一張沙發放進牆的正中間
——**拖曳那條路徑本來完全沒有檢查**。2D 看起來只是畫在牆上面，像是一個決定；3D
是一面牆從沙發中間穿過去。

`core/clearance.ts` 是兩個有向包圍盒的分離軸測試，回傳的不是「有沒有重疊」而是
**最短的推出向量**。三件容易做錯的：

- **四個候選軸，不是兩個。** 只試家具自己的兩個軸的話，方正的牆推得開、30° 的
  斜牆推不開——而它不會報錯，只是沙發留在牆裡
- **一次只推最深的那一個，然後重跑。** 把牆角兩道牆的推力相加會沿對角線衝過頭，
  穿過兩道牆掉進房間裡；一次推一個才會沿著牆面滑
- **迴圈要有上限。** 3m 的沙發塞不進 2m 的凹槽，那沒有合法答案，回最後一個位置
  比掛住誠實

三種東西不算違規：`mount: 'wall'`（壁燈、壁掛電視，它就該在牆上）、
`mount: 'ceiling'`（吊燈，平面上的重疊不是碰撞）、電氣配件（本來就吸在中心線上）。
曲線牆**不推**——用弦當包圍盒會把弧的內側整片當成實心，把家具從房間中央推出去。

規則是推出來而不是擋住，跟第一條同一個取捨：寧可讓使用者看到最後的結果，也不要
讓他對著一個不會成真的畫面瞄準。所以物件可能不在游標底下，但它一定在合法位置。

**四條共用同一個原則：不要超出使用者畫的線。** `bench/verify-placement.mjs` 驗第一
...
### 牆面差 4.5cm：那不是柱子的錯，是厚度不同的兩道牆接在同一條中心線上

使用者回報「柱子跟牆壁在某些邊緣有極小的不平整落差」。實際量 IMG_9720 那份圖，
落差跟柱子無關：**24cm 的結構牆接 15cm 的隔間牆，中心線對齊，於是兩面各露出
(24−15)/2 = 4.5cm 的牆端面**，四處。平面圖上兩個盒子是butt在一起的、看起來連續，
只有 3D 從地板到天花板露出一條窄稜——而那正是使用者形容的樣子。

`findFaceSteps()`（`core/wallEdit.ts`）掃出來，`Editor.alignFaceStep()` 套用。
**門檻 5cm 是這件事能自動跑的原因**：超過它的凸出不是誤差而是東西本身——那根柱子
82cm 厚，比它嵌著的牆凸 10cm 和 48cm，拉平等於把它從底圖量到的黑塊上移開。門檻
以下沒有東西是刻意畫成那樣的。

三件做對了才不會出錯的：

- **兩面各給一顆按鈕，不自動選。** 兩面差一樣多，幾何裡沒有依據可以挑；哪一間房間
  拿到平整的那面是關於這棟房子的決定。按鈕上寫房間名字而不是「左／右」
- **房間名字要從「要動的那道牆自己的中點」往外量**，不是從接頭往外量。接頭是三四道
  牆交會的地方，往旁邊 20cm 常常落在別間房裡：實測 wall_20 從接頭問出來是「主浴」、
  從中點問出來是「主臥室」，後者才是那顆按鈕會弄平的那一面。**名字錯的按鈕比沒有
  名字的按鈕更難用**
- **牆移動時門窗要一起移動。** 開口存的是世界座標不是「沿牆多少公分」，牆自己走了
  4.5cm 就會留下一扇偏心的門——還在牆裡、還是照樣打洞，只是不在框的位置上

這一區在右側面板、沒選取物件時才出現，而且 `findFaceSteps` 掃不到東西它就不存在。
不加工具列按鈕是刻意的：工具列剛因為兩個畫不出來的字符被抱怨過，再多一顆九成時間
沒有用的按鈕是同一個方向的錯。
...
一疊橫線看起來像是故意的。所以按鈕整個靠它撐起來的圖示改成 inline SVG（`.icon`），
不依賴任何字型。有文字標籤在旁邊的（`▥ 分割`、`⊹ 中心`）留著用字型，退化成
「方框＋看得懂的字」還活得下去。
...
- 目前 client 305 個測試、server 110 個
- `alignFaceStep` 移動牆體之後，**尺寸標註不會跟著走**（門窗會）。標註是獨立的
  a/b 座標，沒有指向它量的那道牆——4.5cm 在標註上是噪音級，但這是已知的不一致
- 家具的**縮放**不套用出牆檢查，只有放置／拖曳／方向鍵／旋轉套用。把櫃體拉到貼牆
  正是使用者會刻意做的事，在那條路徑上推開它是幫倒忙
