...
### 家具與裝潢素材：141 件（127 件 CC0 模型 ＋ 14 款參數化衣櫃）

`client/public/models/<catalogueId>/`（glTF ＋ .bin ＋ 貼圖，共 41MB），由
`scripts/fetch_models.py` 從 **Poly Haven** 抓，同樣 **CC0 1.0**。目錄 141 件裡 127 件有模型：Poly Haven 90 件（實掃）、Kenney Furniture Kit 37 件、Quaternius 10 件
...
141 件塞不進一條清單，光客廳就 48 顆按鈕。三個控制項**刻意互相獨立**：搜尋是名稱
子字串、風格是集合（可複選，聯集）、摺疊是每個分類各自記。一旦互相卡住，一個空
面板就有三種可能原因，使用者只能猜。摺疊與風格都存 localStorage——每次重載都把
八個分類全部展開，等於沒有這個功能。
...
`bench/verify-palette.mjs` 驗的是**組合**：搜尋 ∩ 風格、風格複選是聯集、摺疊不改變
