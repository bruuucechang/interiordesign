...
這裡是從 `AGENTS.md` 搬過來的量測、推導、功能規格與逐次除錯紀錄。
**內容一字未改，只是換了位置。**

搬家的理由：`AGENTS.md` 每個 session 都會被完整載入，而這些內容只有在真的要動
對應的系統時才需要讀。搬到這裡之後 context 成本降為零，需要時再讀這一份。

搬移日期 2026-09-08　·　1 節　·　原本佔 `AGENTS.md` 的 500 行
...
## 歷史（undo/redo）的成本，2026-09-08 量的

改成「快照先擱著、`emit()` 時比對」之後，最壞情況是**有一筆 pending 卻一直沒有東西
改變**（例如 focus 了一個欄位就跑去到處點選），這時每一次 `emit()` 都要多做一次
`snapshot()`。量了一份比真實資料大十二倍的圖來確認那不是問題：
...
| 底圖 | 3.0 MB 的 base64 |
| 一次 `commit()` | **0.70 ms** |
| 一次 `settle()`（真的有變、要歸檔） | **0.40 ms** |
| pending 掛著時的空 `emit()` | **0.44 ms** ×50 = 21.8 ms |
| 歷史總大小 | **17.9 KB**，等於底圖的 **0.6%** |

最後一列是內連底圖的效果：**修改前光是一筆快照就超過 4 MB**，而上限是 100 筆，
而描圖正好是會把 100 筆填滿的那個流程。

0.44 ms 落在一幀 16.7 ms 裡面很寬鬆，而且那是最壞情況——正常拖曳在第一幀就把
pending 歸檔了，之後完全不會再 stringify。

---

<!-- 2026-09-16 從 AGENTS.md 搬進來的章節。內容逐字未改，搬遷理由見 claude-skills 的 README「一條規則該放哪一層」。 -->
