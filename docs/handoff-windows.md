...
安裝程式是 **96 MB 的 `.exe`**（255 MB 的資料夾壓下來的），這兩件事跟程式好不好
無關，純粹是它的副檔名與大小：

- **Gmail 直接拒收 `.exe` 附件**，壓成 zip 也一樣（它會看進去）。而且 25 MB 就超過
  附件上限了。
- **Google Drive 對超過 25 MB 的檔案會顯示「無法掃描病毒」**，下載時多一個確認。
  對方會看到第三個看起來像警告的東西。

比較不會出事的做法，照對這個對象的友善程度排：

1. **USB 隨身碟**——沒有任何一關。如果見得到面就用這個。
2. **通訊軟體傳檔**（LINE 上限 1 GB）——對方在他熟悉的介面裡點「下載」，
   不會跳掃毒警告。**但 `.exe` 要先包成 zip，見下一節。**
3. Google Drive／雲端硬碟連結——會多一次「無法掃描病毒，仍要下載」。
   要用的話，先跟他講會看到這一句。

**不要為了繞過 Gmail 而把它改名成 `.txt` 之類的再叫對方改回來。** 那要求他做一件
他做不到的事，而且會讓整件事看起來更可疑。

### LINE 會把 `.exe` 靜默吃掉（2026-09-07 實測）

症狀：寄件端顯示傳送成功，**對方的聊天室裡根本沒有那則訊息**。不是失敗，是不存在
——所以兩邊都不會發現，寄的人以為送到了，收的人不知道有東西要收。

大小不是原因（96 MB 對 1 GB 上限）。成因沒有從 LINE 那邊得到證實，但症狀就是伺服器
端過濾可執行檔的樣子，而**修法不需要知道成因**：讓通道裡跑的東西不是可執行檔。

```bash
# 在裝著安裝程式的目錄下
mkdir -p /tmp/idzip && ln 室內設計繪圖-安裝程式.exe /tmp/idzip/InteriorDesigner-Setup.exe
(cd /tmp/idzip && zip -9 -X -j -q ~/Desktop/室內設計繪圖-安裝程式.zip InteriorDesigner-Setup.exe)
rm -rf /tmp/idzip
```

**壓縮不是為了變小，這點要講清楚。** 實測 100,985,815 → 100,433,453 bytes，**只掉
0.55%**——Inno Setup 已經 `lzma2/max` ＋ `SolidCompression` 壓過一輪，沒有東西可以
再擠。換 7z 或 xz 也一樣。所以任何「壓一壓就寄得出去」的期待，如果講的是大小，
在這個檔案上一律不成立。

**zip 裡面的檔名一定要純 ASCII。** macOS 的 `zip` 把中文檔名以 UTF-8 位元組寫入，
卻**不設 UTF-8 旗標**（general purpose flag 實測 `0x0002`，bit 11 是 0），於是繁中
Windows 照 CP950 解，`室內設計繪圖-安裝程式.exe` 解出來是
`摰文�扯身閮�蝜芸��-摰�鋆�蝔�撘�.exe`。一個未簽章、還要過 SmartScreen 的檔案再配
一個亂碼檔名，就是會讓人不敢點的組合。外層 zip 的檔名維持中文沒問題（LINE 自己
處理 UTF-8 沒有問題），**只有壓縮檔內部的那一個名字要換成 ASCII**。

#### 這個 zip 跟[第三節](#三交付安裝精靈不要發-zip)禁止的 zip 不是同一件事

第三節反對的是把 **`dist\InteriorDesigner\` 那個資料夾**壓成 zip：裡面是
`InteriorDesigner.exe` 加一個 `_internal\`，在 zip 裡直接雙擊會只解出那一個 exe，
程式當場死在找不到 Python DLL，而且畫面上沒有任何字說明少了什麼。

這裡包的是**單一、自足的安裝精靈**。就算對方不解壓縮、直接在 zip 裡雙擊，Windows
會把那一個檔解到暫存目錄再執行，而它不需要旁邊有任何東西——安裝照樣跑完。所以
多出來的「解壓縮」這一步是**可以失敗而不致命的**，第三節那個是不能失敗的。

**如果連 zip 都沒過去**，改傳 Google Drive 連結：連結是純文字，通道不可能濾掉它。
代價是回到上面第 3 項那個「無法掃描病毒」的確認，先跟他講會看到那一句。
